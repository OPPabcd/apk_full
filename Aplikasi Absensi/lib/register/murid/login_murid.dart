...
import 'package:apk/dashboard/d_c_murid/database/auth_ortu.dart';
import 'package:apk/register/page1.dart';

class UserMurid extends StatefulWidget {
  const UserMurid({super.key});

  @override
  State<UserMurid> createState() => _UserMuridState();
}

class _UserMuridState extends State<UserMurid> {
  final AuthOrtu _auth = AuthOrtu();
  ...
  void _login() async {
    ...
    try {
      ...
      final result = await _auth.signInOrtu(emailAdmin, nis);

      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('user_email', emailAdmin);
      await prefs.setString('user_role', 'murid');
      // Daftarkan token FCM ke Supabase
      final muridId = result['id_tabel']?.toString();
