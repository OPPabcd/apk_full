...
class ChatPrivateG extends StatefulWidget {
  ...
  @override
  State<ChatPrivateG> createState() => _ChatPrivateGState();
}

class _ChatPrivateGState extends State<ChatPrivateG> {
  ...
  Future<void> _initChat() async {
    try {
      ...
      _currentGuruId = guruInfo['id_tabel'].toString();
      ...
      if (_currentGuruId != null) {
        if (widget.receiverType == 'guru') {
          _chatStream = supabase
            ...
            .map((data) => data
                .where((e) => 
                    (e['pengirim_guru']?.toString() == _currentGuruId && e['penerima_guru']?.toString() == widget.receiverId) ||
                    (e['pengirim_guru']?.toString() == widget.receiverId && e['penerima_guru']?.toString() == _currentGuruId))
          ...
          _chatStream = supabase
            ...
            .map((data) => data
                .where((e) => 
                    (e['pengirim_guru']?.toString() == _currentGuruId && e['penerima_murid']?.toString() == widget.receiverId) ||
                    (e['pengirim_murid']?.toString() == widget.receiverId && e['penerima_guru']?.toString() == _currentGuruId))
                .map((e) => e as Map<String, dynamic>)
                .toList());
        }
      } else {
        _errorMessage = 'Gagal memuat profil Guru.';
      }
    } catch (e) {
  ...
  Future<void> _sendMessage() async {
    ...
    if (text.isEmpty || _currentGuruId == null) return;
                              ...
                              builder: (context, snapshot) {
                                ...
                                return ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    final isMe = msg['pengirim_guru']?.toString() == _currentGuruId;
            ...
            child: Column(
              ...
              children: [
                ...
                if (idIzin.isNotEmpty) ...[
                  const Divider(),
                  IzinStatusWidget(idIzin: idIzin, isTeacher: true, textPesan: text),
                ],
                const SizedBox(height: 8),
                Align(
...
class _IzinStatusWidgetState extends State<IzinStatusWidget> {
  ...
  Future<void> _updateStatus(bool status) async {
    setState(() => _isLoading = true);
    try {
      // 1. Fetch current status first to prevent duplicate posting
      final currentRes = await Supabase.instance.client
          .from('leave_request')
          .select('verif, id_murid, id_kelas')
          .eq('id_tabel', widget.idIzin)
          .maybeSingle();
      ...
      final bool? currentVerif = currentRes['verif'];
      ...
      // 3. If transitioning to approved (verif was not true, now is true), send to group chat
      if (status == true && currentVerif != true) {
        final idMurid = currentRes['id_murid']?.toString();
        final idKelas = currentRes['id_kelas']?.toString();
        if (idMurid != null && idKelas != null && widget.textPesan != null && widget.textPesan!.isNotEmpty) {
          try {
            await DbGrupKelas().sendMessage(
              text: widget.textPesan!,
              senderId: idMurid,
              role: 'murid',
              idClass: idKelas,
            );
          } catch (e) {
            debugPrint("Gagal meneruskan izin ke grup kelas: $e");
          }
        }
      }
