...
import 'package:apk/dashboard/d_guru/function/f_pesan_g/anggota_g/member_kelas_g.dart';
...
import 'package:apk/dashboard/d_guru/function/f_pesan_g/pengumuman.dart';
import 'package:apk/dashboard/d_guru/function/f_pesan_g/f_notif/grup_kelas_n.dart';

class GrupKelasPageG extends StatefulWidget {
  const GrupKelasPageG({super.key});

  @override
  State<GrupKelasPageG> createState() => _GrupKelasPageGState();
}

class _GrupKelasPageGState extends State<GrupKelasPageG> {
  ...
  Future<void> _initChat() async {
    try {
      ...
      _userNames = await DbGrupKelas.fetchAllNames();

      // Fetch current user info
      final currentUserInfo = await DbGrupKelas.getCurrentUserRoleAndId();
      
      if (currentUserInfo != null) {
        _currentUserId = currentUserInfo['id'];
        _currentUserRole = currentUserInfo['role'];
        _currentClassId = currentUserInfo['id_class'];

        if (_currentUserRole == 'admin') {
           _errorMessage = 'Admin tidak tergabung secara default dalam satu grup kelas spesifik.';
        } else if (_currentClassId == null || _currentClassId!.isEmpty) {
           _errorMessage = 'Anda belum terdaftar di kelas manapun.';
        } else {
           _chatStream = DbGrupKelas().getGrupKelasStream(_currentClassId!);
        }
      } else {
  ...
  Future<void> _sendMessage() async {
    ...
    if (text.isEmpty || _currentUserId == null || _currentUserRole == null || _currentClassId == null) return;
            ...
            child: Column(
              ...
              children: [
                ...
                if (idIzin.isNotEmpty) ...[
                  const Divider(),
                  IzinStatusWidget(idIzin: idIzin, isTeacher: true),
                ],
                const SizedBox(height: 8),
                Align(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Anggota Kelas',
            onPressed: () {
              if (_currentClassId != null && _currentClassId!.isNotEmpty) {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => MemberKelasPageG(classId: _currentClassId!),
                  ),
                );
              } else {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Anda belum tergabung di kelas mana pun')),
                );
              }
            },
          )
                      ...
                      child: _chatStream == null
                          ...
                          : StreamBuilder<List<Map<String, dynamic>>>(
                              ...
                              builder: (context, snapshot) {
                                ...
                                if (messages.isEmpty) {
                                  return const Center(child: Text('Belum ada pesan di kelas ini'));
                                }
                                ...
                                return ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    final senderName = isMe ? 'Anda' : (_userNames[senderId] ?? 'Tidak diketahui');

                                    final textMsg = msg['text'] ?? '';
                                    if (textMsg.startsWith("📋 *Pengajuan Izin*")) {
                                      return _buildIzinCard(textMsg, senderName, _formatTime(msg['created_at']), isMe);
                                    }
                          ...
                          children: [
                            PopupMenuButton<String>(
                              ...
                              onSelected: (value) {
                                if (value == 'image') {
                                  ...
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => PengumumanPage(classId: _currentClassId),
                                    ),
                                  );
                                }
                              },
                              itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                                ...
                                if (_currentUserRole == 'admin' || _currentUserRole == 'guru')
                                  const PopupMenuItem<String>(
                                    value: 'announcement',
                                    child: Row(
                                      children: [
...
class IzinStatusWidget extends StatefulWidget {
  ...
  const IzinStatusWidget({super.key, required this.idIzin, required this.isTeacher});
